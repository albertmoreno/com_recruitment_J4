...
$canCreate  = $user->authorise('core.create', 'com_recruitment') && file_exists(JPATH_COMPONENT . DIRECTORY_SEPARATOR . 'forms' . DIRECTORY_SEPARATOR . 'applicationform.xml');
$canEdit    = $user->authorise('core.edit', 'com_recruitment') && file_exists(JPATH_COMPONENT .  DIRECTORY_SEPARATOR . 'forms' . DIRECTORY_SEPARATOR . 'applicationform.xml');
$canCheckin = $user->authorise('core.manage', 'com_recruitment');
$canChange  = $user->authorise('core.edit.state', 'com_recruitment');
$canDelete  = $user->authorise('core.delete', 'com_recruitment');
	...
	<div class="table-responsive">
		<table class="table table-striped" id="applicationList">
			...
			<tr>
					...
					<th class=''>
						<?php echo HTMLHelper::_('grid.sort',  'COM_RECRUITMENT_APPLICATIONS_FIRSTNAME', 'a.firstname', $listDirn, $listOrder); ?>
					</th>
	...
	if($canDelete) {
		$wa->addInlineScript("
			...
			function deleteItem() {
				...
				if (!confirm(\"" . Text::_('COM_RECRUITMENT_DELETE_MESSAGE') . "\")) {
					return false;
				}
			}
		", [], [], ["jquery"]);
